...
/// An alias for result (borrowed from anyhow)
pub type Result<T, E = SubparError> = core::result::Result<T, E>;
/// And make sure Context exists as well
pub use anyhow::Context as SubparContext;
...
impl From<wrapi::WrapiError> for SubparError {
  fn from(err: wrapi::WrapiError) -> SubparError {
    match err {
      ...
      wrapi::WrapiError::UrlError(_) => SubparError::UrlError,
    }
  }
}
...
impl From<serde_json::Error> for SubparError {
  fn from(_err: serde_json::Error) -> SubparError {
    unimplemented!("'' still needs to be implemented")
  }
}
